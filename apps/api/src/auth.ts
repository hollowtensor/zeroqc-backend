import { sendMagicLinkEmail, sendWorkspaceInvitationEmail } from "@zeroqc/email";
...
const apiUrl = process.env.ZEROQC_API_URL || "http://localhost:1337";
const clientUrl = process.env.ZEROQC_CLIENT_URL || "http://localhost:5173";
...
export const auth = betterAuth({
  ...
  plugins: [
    anonymous({
      ...
      emailDomainName: "zeroqc.com",
    }),
    lastLoginMethod(),
    magicLink({
      sendMagicLink: async ({ email, url }) => {
        try {
          await sendMagicLinkEmail(email, "Login for zeroqc", {
            magicLink: url,
          });
        } catch (error) {
          console.error(error);
    ...
    organization({
      ...
      async sendInvitationEmail(data) {
        const inviteLink = `${process.env.ZEROQC_CLIENT_URL}/auth/accept-invitation/${data.id}`;

        await sendWorkspaceInvitationEmail(
          data.email,
          `${data.inviter.user.name} invited you to join ${data.organization.name} on zeroqc`,
          {
  ...
  advanced: {
    defaultCookieAttributes: {
      ...
      domain: process.env.COOKIE_DOMAIN || undefined, // Optional: e.g., ".hollow.com" for explicit cross-subdomain cookies
    },
  },
});
