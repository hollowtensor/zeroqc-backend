...
This Helm chart deploys [zeroqc](https://zeroqc.app) - open source project management that works for you, not against you.

## Introduction

This chart bootstraps a zeroqc deployment on a Kubernetes cluster using the Helm package manager. It deploys both the API backend and Web frontend components, along with a PostgreSQL database, with optional ingress resources.
...
# Install with Helm
helm install zeroqc ./charts/zeroqc --namespace zeroqc --create-namespace

# Access locally
kubectl port-forward svc/zeroqc-web 5173:5173 -n zeroqc
```
...
```bash
helm install zeroqc ./charts/zeroqc \
  --namespace zeroqc \
  --create-namespace \
...
To install the chart with the release name `my-zeroqc`:

```bash
helm install my-zeroqc ./charts/zeroqc
```

The command deploys zeroqc on the Kubernetes cluster with default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.

## Uninstalling the Chart

To uninstall/delete the `my-zeroqc` deployment:

```bash
helm uninstall my-zeroqc
```
...
| `postgresql.auth.database`          | PostgreSQL database name                                                                                           | `zeroqc`                         |
| `postgresql.auth.username`          | PostgreSQL username                                                                                                | `zeroqc_user`                    |
| `postgresql.auth.password`          | PostgreSQL password                                                                                                | `zeroqc_password`                |
...
| Name                                | Description                                                                                                        | Value                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------- |
| `api.image.repository`              | API image repository                                                                                              | `ghcr.io/usezeroqc/api`          |
...
| `api.env.database.external.database` | External PostgreSQL database name                                                                                | `zeroqc`                         |
| `api.env.database.external.username` | External PostgreSQL username                                                                                     | `zeroqc_user`                    |
...
| Name                                | Description                                                                                                        | Value                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------- |
| `web.image.repository`              | Web image repository                                                                                               | `ghcr.io/usezeroqc/web`          |
...
kubectl create secret generic zeroqc-secrets \
  --namespace zeroqc \
...
- Database name: `zeroqc`
- Username: `zeroqc_user`
- Default password: `zeroqc_password` (change this in production!)
...
kubectl exec -it deployment/my-zeroqc-postgresql -- pg_dump -U zeroqc_user zeroqc > zeroqc-backup.sql
```
...
Contact the zeroqc community on [Discord](https://discord.gg/rU4tSyhXXU) for migration assistance.
...
```bash
kubectl apply -f zeroqc-gateway.yaml
```
